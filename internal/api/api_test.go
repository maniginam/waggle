...
func TestModelProviderAPI(t *testing.T) {
	_, ts := setup(t)
	defer ts.Close()
	...
	resp, err := http.Post(ts.URL+"/api/models", "application/json", bytes.NewBuffer(body))
	if err != nil {
		t.Fatal(err)
	}
	if resp.StatusCode != http.StatusCreated {
	...
	var created model.ModelProvider
	json.NewDecoder(resp.Body).Decode(&created)
	resp.Body.Close()
	if created.Name != "dall-e-3" {
		t.Errorf("expected dall-e-3, got %s", created.Name)
	}
	...
	http.Post(ts.URL+"/api/models", "application/json", bytes.NewBuffer(body2))
	...
	// GET by capability
	resp, _ = http.Get(ts.URL + "/api/models?capability=image-gen")
	var imageModels []model.ModelProvider
	json.NewDecoder(resp.Body).Decode(&imageModels)
	resp.Body.Close()
	if len(imageModels) != 1 || imageModels[0].Name != "dall-e-3" {
		t.Errorf("expected [dall-e-3], got %v", imageModels)
	}

	// GET by id
	resp, _ = http.Get(ts.URL + "/api/models/" + created.ID)
	var gotModel model.ModelProvider
	json.NewDecoder(resp.Body).Decode(&gotModel)
	resp.Body.Close()
	if gotModel.Name != "dall-e-3" {
		t.Errorf("expected dall-e-3, got %s", gotModel.Name)
	}

	// GET by name
	resp, _ = http.Get(ts.URL + "/api/models/claude-sonnet")
	json.NewDecoder(resp.Body).Decode(&gotModel)
	resp.Body.Close()
	if gotModel.Provider != "anthropic" {
		t.Errorf("expected anthropic, got %s", gotModel.Provider)
	}

	// DELETE
	req, _ := http.NewRequest(http.MethodDelete, ts.URL+"/api/models/"+created.ID, nil)
	...
	resp.Body.Close()
	if len(models) != 1 {
		t.Errorf("expected 1 after delete, got %d", len(models))
	}
}

func TestAgentRegistrationWithModel(t *testing.T) {
	_, ts := setup(t)
	defer ts.Close()

	// Create model provider first
	body, _ := json.Marshal(map[string]any{
		"name":         "grok-3",
		"provider":     "xai",
		"capabilities": []string{"code", "text"},
	})
	http.Post(ts.URL+"/api/models", "application/json", bytes.NewBuffer(body))
	...
	resp, _ := http.Post(ts.URL+"/api/agents/register", "application/json", bytes.NewBuffer(agentBody))
	var agent map[string]any
	json.NewDecoder(resp.Body).Decode(&agent)
	resp.Body.Close()
	if agent["model"] != "grok-3" {
		t.Errorf("expected model grok-3, got %v", agent["model"])
	}
	...
	for _, a := range agents {
		if a["name"] == "smart-agent" && a["model"] == "grok-3" {
			found = true
		}
	}
	if !found {
		t.Error("agent with model grok-3 not found in list")
	}
}
...
func TestModelValidation(t *testing.T) {
	...
	resp, _ := http.Post(ts.URL+"/api/models", "application/json", bytes.NewBuffer(body))
	if resp.StatusCode != http.StatusBadRequest {
		t.Errorf("expected 400 for missing name, got %d", resp.StatusCode)
	}
	resp.Body.Close()
	...
	// Missing provider
	body, _ = json.Marshal(map[string]string{"name": "test-model"})
	resp, _ = http.Post(ts.URL+"/api/models", "application/json", bytes.NewBuffer(body))
	if resp.StatusCode != http.StatusBadRequest {
	...
	// Not found
	resp, _ = http.Get(ts.URL + "/api/models/nonexistent")
	if resp.StatusCode != http.StatusNotFound {
		t.Errorf("expected 404, got %d", resp.StatusCode)
	}
	resp.Body.Close()
